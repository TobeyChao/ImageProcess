---
name: gen-image
description: 使用 Gemini API 或阿里云百炼 Wan2.7 Pro 生成图片。当用户想生成图片、画图、创建图像、制作图、出图时必须触发此 skill。触发词包括：生成图片、画一张、帮我画、生图、generate image、create image、make an image、draw a picture 等。即使用户描述很简短（如"画只猫"、"来张风景图"），也应触发此 skill。
compatibility: 依赖 Gemini API 密钥（GEMINI_API_KEY）或阿里云百炼 API 密钥（DASHSCOPE_API_KEY），需联网，无需 GPU。
---

## 目的

调用图像生成脚本（支持 Gemini 或 Wan2.7 Pro），将用户的描述转化为实际图片并保存到项目输出目录。
...
# 使用 Wan2.7 Pro
python .claude/skills/gen-image/scripts/gen_image.py -m wan -p "<优化后的英文prompt>" [-r <宽高比>] [-s <分辨率>]
```

只有非默认值才需要传 `-r` 和 `-s`，输出目录默认为 `local/output`，无需额外指定。

**模型选择**：若用户未指定模型，默认用 Gemini。若用户提到"万象"、"wan"、"阿里"、"百炼"，则传 `-m wan`。Wan2.7 不支持 `512` 分辨率，会自动映射为 `1K`。
...
| 错误特征 | 可能原因 | 建议 |
|---------|---------|------|
| `User location is not supported` | 当前地区不支持 Gemini 图片生成 API | 尝试 `-m wan` 切换 Wan2.7，或使用代理 |
| `API key not valid` | 密钥过期或被吊销 | 更新 `.claude/settings.local.json` 中的对应 API Key |
...
- Gemini API 密钥通过 `GEMINI_API_KEY` 环境变量配置
- Wan2.7 API 密钥通过 `DASHSCOPE_API_KEY` 环境变量配置
- 两者均在 `.claude/settings.local.json` 中设置
- 需要联网，API 有频率限制
- 同时要多张图时，依次多次运行脚本
