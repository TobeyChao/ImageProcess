...
| 平台 | 加速方式 | 前提条件 |
|------|---------|---------|
| NVIDIA | CUDA | PyTorch CUDA 版本匹配 |
| AMD（Windows） | ROCm | Python 3.12，Windows 专属 ROCm torch 包 |
| Apple Silicon | MPS | macOS 12.3+，M1/M2/M3 芯片 |
| 通用回退 | CPU | 自动，无需配置 |

向导启动时自动检测 GPU；若检测到 NVIDIA 但缺 CUDA torch，会提供一键安装入口。AMD ROCm 用户需使用 Python 3.12 并手动安装 ROCm 专属 torch 包（向导会给出命令）。Apple Silicon 用户 MPS 加速由 PyTorch 自动启用，向导无需额外操作。
...
```
无需 GPU，仅依赖 pillow + numpy。`-c` 支持 Hex 格式（如 `#FFFFFF`）；不传 `-c` 或传入 `auto` 时自动检测背景色（四角采样中位数）。容差 `-t` 默认 32，适合大多数近纯色背景；值越大抠除范围越宽。输出 RGBA PNG。
